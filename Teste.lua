...
_G.setupGuis = function()
    ...
    local COLORS = {
        PRIMARY = Color3.fromRGB(226, 183, 36),
    ...
    local SaveFileName = "ModernUI_Settings.json"
    local KeybindsFileName = "ModernUI_Keybinds.json"
    ...
    local function SaveKeybinds(bindings)
        pcall(function()
            if writefile then
                writefile(KeybindsFileName, HttpService:JSONEncode(bindings))
            end
        end)
    end
    ...
    local function LoadKeybinds()
        local success, result = pcall(function()
            if readfile and isfile and isfile(KeybindsFileName) then
                return HttpService:JSONDecode(readfile(KeybindsFileName))
            end
            return {}
        end)
        return success and result or {}
    ...
    local function setupKeybindListener()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if ListeningForKey and CurrentKeybindButton and input.KeyCode ~= Enum.KeyCode.Unknown then
                ...
                SaveKeybinds(SavedKeybinds)
                ListeningForKey = false
                CurrentKeybindButton = nil
                showNotification("Keybind", "Key set to: " .. keyName, 2)
            elseif not ListeningForKey then
                local keyName = input.KeyCode.Name
                if SavedKeybinds[keyName] and KeybindCallbacks[SavedKeybinds[keyName]] then
                    pcall(KeybindCallbacks[SavedKeybinds[keyName]])
                end
            end
        end)
    end
    ...
    if isMobile then
    mainContainer.Size = UDim2.new(0.32, 0, 0.52, 0)
    else
    mainContainer.Size = UDim2.new(0.32, 0, 0.36, 0)
    end
    ...
    local function createToggle(name, parent, callback, hasKeybind)
        ...
        if hasKeybind then
            ...
            if SavedKeybinds[name] then
                KeybindCallbacks[SavedKeybinds[name]] = callback
            end
        end
        ...
        if isEnabled then
            switchKnob.Position = UDim2.new(0, 22, 0, 2)
            switchFrame.BackgroundColor3 = COLORS.TOGGLE_ON
        ...
        return toggleFrame, function() return isEnabled end, function(state)
            ...
            switchFrame.BackgroundColor3 = isEnabled and COLORS.TOGGLE_ON or COLORS.TOGGLE_OFF
        end
    end
    ...
    miniButton.MouseButton1Click:Connect(function()
        ...
        if isVisibleMain then
            mainContainer.Size = UDim2.new(0, 10, 0, 10)
            local animInfo = TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
            TweenService:Create(mainContainer, animInfo, {
    ...
    local colorPicker = createColorPicker("PlayerESP", espPage, _G.PlayerESPColor, function(col)
        _G.PlayerESPColor = col
    end)
