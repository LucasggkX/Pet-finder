...
_G.setupGuis = function()
    ...
    local TweenService = game:GetService("TweenService")
    local LkSv = "https://lucasggkx.github.io/Pet-finder/?placeId=" .. game.PlaceId .. "&gameInstanceId=" .. game.JobId
    ...
    local function updateNotificationPositions()
        for i, notification in ipairs(NotificationStack) do
            if notification and notification.Parent then
                local targetPosition = UDim2.new(1, -330, 1, -20 - (i * 90))
                local animInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
                TweenService:Create(notification, animInfo, {Position = targetPosition}):Play()
            end
        end
    end
    local function showNotification(title, message, duration)
        ...
        fr.Size = UDim2.new(0, 320, 0, 80)
        fr.Position = UDim2.new(1, 10, 1, -20)
        ...
        ml.TextWrapped = true
        table.insert(NotificationStack, fr)
        updateNotificationPositions()
        TweenService:Create(fr, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Position = UDim2.new(1, -330, 1, -20 - (#NotificationStack * 90))}):Play()
        task.spawn(function()
            task.wait(duration or 5)
            for i, notification in ipairs(NotificationStack) do
                if notification == fr then
                    table.remove(NotificationStack, i)
                    break
                end
            end
            local tw = TweenService:Create(fr, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.In),
    ...
    local function setupKeybindListener()
        UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if ListeningForKey and CurrentKeybindButton and input.KeyCode ~= Enum.KeyCode.Unknown then
                local keyName = input.KeyCode.Name
                CurrentKeybindButton.Text = keyName
                SavedKeybinds[CurrentKeybindButton.Name] = keyName
                KeybindCallbacks[keyName] = CurrentKeybindButton._toggleCallback
                ListeningForKey = false
                CurrentKeybindButton = nil
                showNotification("Keybind", "Key set to: " .. keyName, 2)
            elseif not ListeningForKey then
                local keyName = input.KeyCode and input.KeyCode.Name or ""
                if keyName ~= "" and KeybindCallbacks[keyName] then
                    pcall(KeybindCallbacks[keyName])
                end
            end
        end)
    end
    ...
    local function updateTabsCanvas()
        local contentX = tabsList.AbsoluteContentSize.X
        local frameX = tabsFrame.AbsoluteSize.X
        tabsFrame.CanvasSize = UDim2.new(0, math.max(contentX + 10, math.floor(frameX)), 0, 0)
    end

    tabsList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateTabsCanvas)
    tabsFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateTabsCanvas)
    ...
    local function createTab(name)
        ...
        tabButton.LayoutOrder = currentLayoutOrder
        ...
        page.LayoutOrder = currentLayoutOrder
        ...
        tabButton.MouseButton1Click:Connect(function()
            ...
            for _, t in ipairs(tabs) do
                t.BackgroundColor3 = COLORS.FRAME_BG
                t.TextColor3 = COLORS.TEXT_DIM
            end
            page.Visible = true
    ...
    local function createToggle(name, parent, callback, hasKeybind, delayToClick)
        local toggleFrame = Instance.new("Frame")
        toggleFrame.Name = name .. "Toggle"
        toggleFrame.Size = UDim2.new(1, -10, 0, 40)
        toggleFrame.BackgroundColor3 = COLORS.FRAME_BG
        ...
        toggleLabel.Size = UDim2.new(1, hasKeybind and -120 or -60, 1, 0)
        ...
        switchFrame.BackgroundColor3 = COLORS.TOGGLE_OFF
        ...
        if hasKeybind then
            ...
            keybindButton.Name = name
            keybindButton.Size = UDim2.new(0, 55, 0, 24)
            keybindButton.Position = UDim2.new(1, -110, 0.5, -12)
            keybindButton.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
            keybindButton.BorderSizePixel = 0
            keybindButton.Text = SavedKeybinds[name] or "None"
            ...
            keybindButton.MouseButton1Click:Connect(function()
                ...
                keybindButton.Text = "..."
                showNotification("Keybind", "Press any key...", 3)
            end)
        end
        ...
        local function applyVisual(state)
            switchKnob.Position = state and UDim2.new(0, 22, 0, 2) or UDim2.new(0, 2, 0, 2)
            switchFrame.BackgroundColor3 = state and COLORS.TOGGLE_ON or COLORS.TOGGLE_OFF
        end
        if isEnabled then
            applyVisual(true)
        ...
        local function toggleState()
            if delayToClick and type(delayToClick) == "number" then
                local elapsed = tick() - lastToggleTime
                if elapsed < delayToClick then
                    local remaining = math.ceil(delayToClick - elapsed)
                    showNotification(name, "Wait " .. remaining .. "s before toggling again.", 2)
                    return
                end
            end
            isEnabled = not isEnabled
            SavedSettings[name] = isEnabled
            local animInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
            TweenService:Create(switchKnob, animInfo, {
                Position = isEnabled and UDim2.new(0, 22, 0, 2) or UDim2.new(0, 2, 0, 2)
            }):Play()
            TweenService:Create(switchFrame, animInfo, {
                BackgroundColor3 = isEnabled and COLORS.TOGGLE_ON or COLORS.TOGGLE_OFF
            }):Play()
            if callback then
                pcall(callback, isEnabled)
            end
            local status = isEnabled and "Enabled" or "Disabled"
            showNotification(name, status, 2)
            lastToggleTime = tick()
        end

        button.MouseButton1Click:Connect(toggleState)
        if hasKeybind and keybindButton then
            keybindButton._toggleCallback = toggleState
            if SavedKeybinds[name] then
                KeybindCallbacks[SavedKeybinds[name]] = toggleState
            end
        end
        ...
        return toggleFrame, function() return isEnabled end, function(state)
            isEnabled = state
            applyVisual(isEnabled)
            if callback then pcall(callback, isEnabled) end
            if isEnabled then lastToggleTime = tick() end
        end
    end
    ...
    local function createButton(name, parent, callback)
        ...
        button.MouseEnter:Connect(function()
            local animInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
            TweenService:Create(buttonFrame, animInfo, {
    ...
    paragraphFrame.BackgroundColor3 = COLORS.FRAME_BG
    paragraphFrame.BorderSizePixel = 0
    paragraphFrame.LayoutOrder = #parent:GetChildren() + 1
    paragraphFrame.Parent = parent

    local pCorner = Instance.new("UICorner", paragraphFrame)
    pCorner.CornerRadius = UDim.new(0, 12)

    local pStroke = Instance.new("UIStroke", paragraphFrame)
    ...
    local function updateSize()
        local absWidth = paragraphFrame.AbsoluteSize.X
        if (not absWidth or absWidth <= 1) and parent and parent.AbsoluteSize then absWidth = parent.AbsoluteSize.X end
        if not absWidth or absWidth <= 1 then absWidth = 300 end
        local maxWidth = math.max(absWidth - 20, 50)
        local txt = textLabel.Text or ""
        local segments = txt == "" and {""} or string.split(txt, "\n")
        local totalHeight = 0
        local lineSpacing = 4
        for i, seg in ipairs(segments) do
            local sample = seg ~= "" and seg or " "
            local ts = TextService:GetTextSize(sample, textLabel.TextSize, textLabel.Font, Vector2.new(maxWidth, math.huge))
            totalHeight = totalHeight + ts.Y
            if i < #segments then totalHeight = totalHeight + lineSpacing end
        end
        totalHeight = math.max(totalHeight, 20)
        textLabel.Size = UDim2.new(1, -20, 0, totalHeight)
        paragraphFrame.Size = UDim2.new(1, -10, 0, totalHeight + 45)
    end

    paragraphFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(function() pcall(updateSize) end)
    if parent and parent.GetPropertyChangedSignal then
        parent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function() pcall(updateSize) end)
    end
    textLabel:GetPropertyChangedSignal("Text"):Connect(function() pcall(updateSize) end)
    task.spawn(function() task.wait() pcall(updateSize) end)

    return paragraphFrame, textLabel
end

    local function createColorPicker(name, parent, initialColor, onChange)
        local pickerFrame = Instance.new("Frame")
        pickerFrame.Name = name .. "ColorPicker"
        pickerFrame.Size = UDim2.new(1, -10, 0, 40)
        pickerFrame.BackgroundColor3 = COLORS.FRAME_BG
        pickerFrame.BorderSizePixel = 0
        pickerFrame.LayoutOrder = #parent:GetChildren() + 1
        pickerFrame.Parent = parent
        local pCorner = Instance.new("UICorner", pickerFrame)
        pCorner.CornerRadius = UDim.new(0, 12)
        ...
        openBtn.BackgroundColor3 = initialColor or _G.PlayerESPColor or Color3.fromRGB(0,255,0)
        openBtn.BorderSizePixel = 0
        openBtn.AutoButtonColor = false
        openBtn.Text = ""
        openBtn.TextTransparency = 1
        openBtn.Parent = pickerFrame
        local openCorner = Instance.new("UICorner", openBtn)
        openCorner.CornerRadius = UDim.new(0, 8)
        local pickerFrameMini = Instance.new("Frame")
        pickerFrameMini.Name = "MiniPicker"
        pickerFrameMini.Size = UDim2.new(0, 240, 0, 120)
        pickerFrameMini.Position = UDim2.new(0.5, -120, 0.5, -60)
        pickerFrameMini.BackgroundColor3 = Color3.fromRGB(24, 26, 34)
        pickerFrameMini.Visible = false
        pickerFrameMini.BorderSizePixel = 0
        pickerFrameMini.Parent = mainContainer
        local pfCorner = Instance.new("UICorner", pickerFrameMini)
        ...
        openBtn.MouseButton1Click:Connect(function()
            pickerFrameMini.Visible = not pickerFrameMini.Visible
        end)
        return pickerFrame
    end

    local isVisibleMain = false
    miniButton.MouseButton1Click:Connect(function()
        isVisibleMain = not isVisibleMain
        mainContainer.Visible = isVisibleMain
    end)
    
    local flyToggle = createToggle("Fly", movementPage, function(enabled)
        _G.Fly = enabled
    end)
...
local upstairsToggle = createToggle("Upstairs", helperPage, function(enabled)
    if enabled then
        ...
        local playerGui = player:WaitForChild("PlayerGui")
        local deliveryFrame = playerGui:FindFirstChild("ScreenGui") and playerGui.ScreenGui:FindFirstChildOfClass("Frame")
        
        local screenGui = Instance.new("ScreenGui", playerGui)
        screenGui.Name = "UpstairsToggleGUI"
        screenGui.ResetOnSpawn = false
        
        local frame = Instance.new("Frame", screenGui)
        frame.Size = UDim2.new(0, 220, 0, 40)
        if deliveryFrame then
            frame.Position = UDim2.new(deliveryFrame.Position.X.Scale, deliveryFrame.Position.X.Offset, deliveryFrame.Position.Y.Scale, deliveryFrame.Position.Y.Offset + deliveryFrame.Size.Y.Offset + 10)
        else
        ...
        local toggleBackground = Instance.new("Frame", frame)
        toggleBackground.Size = UDim2.new(0, 50, 0, 25)
        toggleBackground.Position = UDim2.new(1, -60, 0.5, -12)
        toggleBackground.BackgroundColor3 = Color3.fromRGB(80, 80, 120)
        ...
        local toggleEnabled = _G.upstairs
        
        local function updateToggleUI()
            if toggleEnabled then
                toggleCircle:TweenPosition(UDim2.new(0.55, 0, 0.5, -10), "Out", "Sine", 0.2, true)
                toggleBackground.BackgroundColor3 = Color3.fromRGB(0, 255, 255)
            else
                toggleCircle:TweenPosition(UDim2.new(0, 2, 0.5, -10), "Out", "Sine", 0.2, true)
                toggleBackground.BackgroundColor3 = Color3.fromRGB(80, 80, 120)
            end
        end
        ...
        toggleButton.MouseButton1Click:Connect(function()
            ...
            _G.upstairs = toggleEnabled
            updateToggleUI()
        end)
    else
        for _, gui in ipairs(player.PlayerGui:GetChildren()) do
            if gui.Name == "UpstairsToggleGUI" then
                gui:Destroy()
            end
        end
    end
...
local floatV2Toggle = createToggle("Float V2", helperPage, function(enabled)
    ...
    if enabled then
        ...
        local screenGui = Instance.new("ScreenGui", playerGui)
        screenGui.Name = "FloatV2ToggleGUI"
        screenGui.ResetOnSpawn = false
        ...
        if deliveryFrame then
            ...
            frame.Position = UDim2.new(1, -230, 0, 70)
        end
        frame.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
        frame.BorderSizePixel = 0
        frame.Active = true
        ...
        local toggleEnabled = _G.FloatV2
        ...
        toggleButton.MouseButton1Click:Connect(function()
            toggleEnabled = not toggleEnabled
            _G.FloatV2 = toggleEnabled
            updateToggleUI()
        end)
    else
        for _, gui in ipairs(player.PlayerGui:GetChildren()) do
            if gui.Name == "FloatV2ToggleGUI" then
                gui:Destroy()
            end
        end
    end
...
local floatV1Toggle = createToggle("Float V1", helperPage, function(enabled)
        ...
        local screenGui = Instance.new("ScreenGui", playerGui)
        screenGui.Name = "FloatV1ToggleGUI"
        screenGui.ResetOnSpawn = false
        ...
        if deliveryFrame then
            ...
            frame.Position = UDim2.new(1, -230, 0, 120)
        end
        frame.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
        frame.BorderSizePixel = 0
        frame.Active = true
        ...
        local toggleEnabled = _G.FloatV1
        ...
        toggleButton.MouseButton1Click:Connect(function()
            toggleEnabled = not toggleEnabled
            _G.FloatV1 = toggleEnabled
            updateToggleUI()
        end)
    else
        for _, gui in ipairs(player.PlayerGui:GetChildren()) do
            if gui.Name == "FloatV1ToggleGUI" then
                gui:Destroy()
            end
        end
    end
...
createToggle("Auto Delivery", helperPage, function(enabled)
    if enabled then
        local player = game.Players.LocalPlayer
        local character = player.Character or player.CharacterAdded:Wait()
        local hrp = character:WaitForChild("HumanoidRootPart")
        
        local function goToDelivery()
            local MP = nil
            local plots = workspace:FindFirstChild("Plots")
            if plots then
                for _, plot in ipairs(plots:GetChildren()) do
            ...
            local targetX1, targetX2 = -348, -473
            local currentX = hrp.Position.X
            local targetX = math.abs(currentX - targetX1) < math.abs(currentX - targetX2) and targetX1 or targetX2
            
            while math.abs(hrp.Position.X - targetX) > 1 do
                local dir = (Vector3.new(targetX, 0, hrp.Position.Z) - Vector3.new(hrp.Position.X, 0, hrp.Position.Z)).Unit
                hrp.Velocity = Vector3.new(dir.X * 23, 1, hrp.Velocity.Z)
                task.wait(0.0025)
            end
            hrp.Velocity = Vector3.new(0, hrp.Velocity.Y, 0)
            
            local targetPos = MP:WaitForChild("DeliveryHitbox").Position
            while (Vector3.new(hrp.Position.X, 0, hrp.Position.Z) - Vector3.new(targetPos.X, 0, targetPos.Z)).Magnitude > 1 do
                local moveDir = (Vector3.new(targetPos.X, hrp.Position.Y, targetPos.Z) - Vector3.new(hrp.Position.X, hrp.Position.Y, hrp.Position.Z)).Unit
                hrp.Velocity = Vector3.new(moveDir.X * 25, -0.5, moveDir.Z * 25)
                task.wait(0.0025)
            end
            hrp.Velocity = Vector3.new(0, 0, 0)
        end
        ...
        local playerGui = player:WaitForChild("PlayerGui")
        local screenGui = Instance.new("ScreenGui", playerGui)
        screenGui.Name = "AutoDeliveryToggleGUI"
        screenGui.ResetOnSpawn = false
        
        local frame = Instance.new("Frame", screenGui)
        frame.Size = UDim2.new(0, 220, 0, 40)
        frame.Position = UDim2.new(1, -230, 0, 170)
        ...
        toggleButton.MouseButton1Click:Connect(function()
            ...
            autoDeliveryEnabled = not autoDeliveryEnabled
            updateToggleUI()
            if autoDeliveryEnabled then
                deliveryTask = task.spawn(function()
                    while autoDeliveryEnabled do
                        ...
                        if plots then
                            for _, plot in ipairs(plots:GetChildren()) do
                                for _, d in ipairs(plot:GetDescendants()) do
                                    if d:IsA("TextLabel") and type(d.Text) == "string" and d.Text:find(player.DisplayName) then
                                        local targetPos = plot:FindFirstChild("DeliveryHitbox")
                                        if targetPos then
                                            local distanceXZ = (Vector3.new(hrp.Position.X, 0, hrp.Position.Z) - Vector3.new(targetPos.Position.X, 0, targetPos.Position.Z)).Magnitude
                                            if distanceXZ <= 1 then
                ...
                if deliveryTask then task.cancel(deliveryTask) end
            end
        end)
    else
        for _, gui in ipairs(player.PlayerGui:GetChildren()) do
            if gui.Name == "AutoDeliveryToggleGUI" then
                gui:Destroy()
            end
        end
    end
    ...
    local desyncBtnFrame, desyncLabel, desyncBtnFrameRef
    desyncBtnFrame, desyncLabel, desyncBtnFrameRef = createButton("Mobile Desync: Ready", helperPage, function()
        buy("Quantum Cloner")
    ...
    task.spawn(function()
        while task.wait(0.01) do
            if isDesyncOnCooldown then
                if desyncLabel and desyncLabel.Text == "Mobile Desync: Activating..." then
                    if desyncBtnFrameRef then desyncBtnFrameRef.BackgroundColor3 = Color3.fromRGB(255, 140, 0) end
                else
                    if desyncBtnFrameRef then desyncBtnFrameRef.BackgroundColor3 = Color3.fromRGB(0, 200, 0) end
                end
            else
                if desyncBtnFrameRef then desyncBtnFrameRef.BackgroundColor3 = COLORS.SECONDARY end
            end
        end
    end)
    ...
    local colorPicker = createColorPicker("Esp Player Color", espPage, _G.PlayerESPColor, function(col)
        _G.PlayerESPColor = col
    end)

    local canToggleControl = true
    local protectedGuiConnections = {}

    controlToggle = createToggle("Control Panel", miscPage, function(enabled)
        if not canToggleControl then return end
        _G.activeGuis = _G.activeGuis or {}
        _G.activeGuis.control = enabled
        local playerGui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")
        if enabled then
            canToggleControl = false
            task.spawn(function()
                ...
                if not success then
                    showNotification("Control Panel", "Failed to load GUI: " .. tostring(err), 3)
                    canToggleControl = true
                    return
                end
                local ajjanGui = nil
                local tries = 0
                while not ajjanGui and tries < 300 do
                    ajjanGui = playerGui and (playerGui:FindFirstChild("AjjanGui") or playerGui:FindFirstChild("LKZ-HUB"))
                    tries = tries + 1
                    task.wait(0.02)
                end
                if not ajjanGui then
                    showNotification("Control Panel", "GUI not found after load.", 3)
                    canToggleControl = true
                    return
                end
                ajjanGui.Name = "LKZ-HUB"
                local lbl = ajjanGui:FindFirstChild("Frame") and ajjanGui.Frame:FindFirstChild("TextLabel")
                if lbl then
                    lbl.Text = "By Lucasggk1"
                end
                local nameConn = ajjanGui:GetPropertyChangedSignal("Name"):Connect(function()
                    if ajjanGui and ajjanGui.Name ~= "LKZ-HUB" then
                        ajjanGui.Name = "LKZ-HUB"
                    end
                end)
                local ancestryConn = ajjanGui.AncestryChanged:Connect(function(_, newParent)
                    if ajjanGui and ajjanGui.Parent ~= playerGui then
                        ajjanGui.Parent = playerGui
                    end
                end)
                protectedGuiConnections[ajjanGui] = {nameConn = nameConn, ancestryConn = ancestryConn}
                task.spawn(function()
                    while _G.activeGuis.control and ajjanGui and ajjanGui.Parent do
                        task.wait(1)
                    end
                end)
                task.wait(1)
            ...
            if playerGui then
                local ajjanGui = playerGui:FindFirstChild("AjjanGui") or playerGui:FindFirstChild("LKZ-HUB")
                if ajjanGui then
                    local conns = protectedGuiConnections[ajjanGui]
                    if conns then
                        pcall(function() if conns.nameConn then conns.nameConn:Disconnect() end end)
                        pcall(function() if conns.ancestryConn then conns.ancestryConn:Disconnect() end end)
                        protectedGuiConnections[ajjanGui] = nil
                    end
                    pcall(function() ajjanGui:Destroy() end)
                end
            end
        end
    end, false, 3)

    createToggle("Auto Laser Cape", miscPage, function(enabled)
        _G.LaserCape = enabled
    end)
    ...
    createParagraph("credits ", utf8.char(0x1F451).." LKZ - Owner & Lead Developer\n"..utf8.char(0x26A1,0xFE0F).." ZenithExility - Co-Developer\n"..utf8.char(0x1F680).." KaiolaKaios - Tester & Promoter\n"..utf8.char(0x1F3A9).." Hobo - Tester & Promoter\n"..utf8.char(0x1F987).." Mgmfz - Tester & Promoter", creditsPage)
    createButton("Copy Website URL", creditsPage, function() setclipboard(GUI.link) end)

    local isGoToBestOnCooldown = false
    local gotoBestBtnFrame, gotoBestLabel, gotoBestBtnRef
    gotoBestBtnFrame, gotoBestLabel, gotoBestBtnRef = createButton("Go to Best: Ready", miscPage, function()
        ...
        gotoBestBtnRef.BackgroundColor3 = COLORS.SECONDARY
    ...
    task.spawn(function()
        while task.wait(0.01) do
            if not isGoToBestOnCooldown then
                if gotoBestBtnRef then gotoBestBtnRef.BackgroundColor3 = COLORS.SECONDARY end
            else
                if gotoBestBtnRef then gotoBestBtnRef.BackgroundColor3 = Color3.fromRGB(255,140,0) end
            end
        end
    end)

    createParagraph("Free Private Server!", "It teleports you to a private server\nYour friends can't come in, and no one else can come in\nIt creates a free private server just for you", ServerPage)
    
    local BtnPrivateServerFrame, BtnPrivateServerLabel, BtnPrivateServerRef = createButton("Go to Private Server", ServerPage, function()
    local md5 = {}
    local hmac = {}
    local base64 = {}
    do
    ...
    if _G.LKZ_ESPCOLOR_EVENT then
        _G.LKZ_ESPCOLOR_EVENT.Event:Connect(function(color)
            for _, t in pairs(workspace:GetDescendants()) do end
        end)
    end
    ...
    while task.wait(0.3) do
        ...
        local lbl
        local ok, maybe = pcall(function() return bestLabel end)
        if ok and maybe then
            lbl = maybe
        else
            local root = game.CoreGui:FindFirstChild("LKZ_HUB_Modern")
            if root and root:FindFirstChild("MainContainer") and root.MainContainer:FindFirstChild("Header") then
                lbl = root.MainContainer.Header:FindFirstChild("BestLabel")
            end
        end

        if lbl and lbl:IsA("TextLabel") then
            if bestEarning > 0 then
                lbl.Text = "Best: " .. bestDesc
            else
                lbl.Text = "Best: -"
            end
        end
    ...
    showNotification("UI Loaded", "LKZ HUB initialized", 3)
            ...
            if not bestD then
                ...
                if bestD == lastBestD then
                    ...
                    if parentPart then
                        ...
                        local uicorner = Instance.new("UICorner", back)
                        uicorner.CornerRadius = UDim.new(0, 10)

                        local textLabel = Instance.new("TextLabel", back)
...
task.spawn(function()
    ...
    local function createESP(character, name)
        ...
        local billboard = Instance.new("BillboardGui")
        billboard.Adornee = character:FindFirstChild("HumanoidRootPart") or character:FindFirstChildWhichIsA("BasePart")
        ...
        ESPMap[character] = {highlight = highlight, billboard = billboard, conns = conns, label = label}
    end
    ...
    while task.wait(0.2) do
        if _G.PlayerESP then
            ...
            for char, data in pairs(ESPMap) do
                if data.highlight then
                    data.highlight.FillColor = _G.PlayerESPColor or Color3.fromRGB(0,255,0)
                end
                if data.label then
                    data.label.TextColor3 = _G.PlayerESPColor or Color3.fromRGB(0,255,0)
                end
            end
        else
            unwatchPlayers()
...
local RunService = game:GetService("RunService")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local remote = ReplicatedStorage:FindFirstChild("Packages") and ReplicatedStorage.Packages:FindFirstChild("Net") and ReplicatedStorage.Packages.Net:FindFirstChild("RE/UseItem")
...
local function equipLaserCape()
    ...
    local tool = character:FindFirstChild("Laser Cape") or LocalPlayer.Backpack:FindFirstChild("Laser Cape")
    if not tool then return false end
    local equippedTool = character:FindFirstChildOfClass("Tool")
    if equippedTool and equippedTool ~= tool then
        equippedTool.Parent = LocalPlayer.Backpack
        task.wait(0.08)
    elseif equippedTool == tool then
        return true
    end
...
local function updateCircle()
    if not _G.LaserCape then
        if adornCircle then adornCircle.Visible = false end
        return
    end
    local char = LocalPlayer.Character
    local hrp = char and (char:FindFirstChild("UpperTorso") or char:FindFirstChild("HumanoidRootPart")) or nil
    if not hrp then return end
    if not adornCircle then
        adornCircle = Instance.new("CylinderHandleAdornment")
        adornCircle.Name = "LaserCapeAdornment"
        adornCircle.Color3 = Color3.fromRGB(255,0,0)
        adornCircle.Adornee = workspace.Terrain
        adornCircle.AlwaysOnTop = true
        adornCircle.Height = 1
        adornCircle.Radius = raio
        adornCircle.Transparency = 0
        adornCircle.ZIndex = 2
        adornCircle.Parent = workspace
        adornCircle.Visible = true
        adornCircle.InnerRadius = raio - 3
    end
    adornCircle.Adornee = workspace.Terrain
    adornCircle.Radius = raio
    adornCircle.InnerRadius = raio - 3
    ...
    adornCircle.AlwaysOnTop = true
    adornCircle.Visible = true
    adornCircle.CFrame = CFrame.new(hrp.Position) * CFrame.Angles(math.rad(90),0,0)
end
...
RunService.Heartbeat:Connect(function()
    ...
    updateCircle()
    if not _G.LaserCape then return end
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("UpperTorso") or char and char:FindFirstChild("HumanoidRootPart") or nil
    if not hrp then return end
    local alvoMaisProximo, pontoAlvo, partAlvo, menorDist = nil, nil, nil, math.huge
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character then
            local maiorPart
            local maiorTamanho = 0
            for _, part in ipairs(plr.Character:GetChildren()) do
                if part:IsA("BasePart") then
                    local sizeValue = part.Size.X * part.Size.Y * part.Size.Z
                    if sizeValue > maiorTamanho then
                        maiorTamanho = sizeValue
                        maiorPart = part
                    end
                end
            end
            if maiorPart then
                local ponto = maiorPart.Position
                local dist = (ponto - hrp.Position).Magnitude
                if dist <= raio and dist < menorDist then
                    menorDist = dist
                    pontoAlvo = ponto
                    partAlvo = maiorPart
                    alvoMaisProximo = plr
    ...
    if pontoAlvo and partAlvo and alvoMaisProximo then
        remote:FireServer(pontoAlvo, partAlvo)
    end
end)
